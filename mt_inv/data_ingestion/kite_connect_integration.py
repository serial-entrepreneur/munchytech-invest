...
access_token_path = project_root / "mt_inv" / "config" / "access_token.json"
...
def save_access_token(token: str, expiry: Optional[str] = None):
...
def load_access_token() -> Optional[str]:
    ...
    try:
        with open(access_token_path, "r") as f:
            data = json.load(f)
        # Optionally, check expiry here if expiry is stored
...
def authenticate() -> bool:
    ...
    if access_token:
        try:
            ...
            kite.profile()
            logger.info("Loaded existing access token. No login required.")
            return True
        except Exception as e:
            logger.warning(f"Stored access token invalid or expired: {e}")
    ...
    try:
        ...
        logger.info("Authentication successful and access token saved.")
        return True
    except Exception as e:
...
def validate_historical_data(data: List[Dict]) -> bool:
    ...
    for point in data:
        ...
        if not (
            ...
            and point["low"] <= point["close"] <= point["high"]
        ):
